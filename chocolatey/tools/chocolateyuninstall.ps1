...
$key = Get-UninstallRegistryKey -SoftwareName 'CutGeek*'
...
elseif ($key.Count -eq 0) {
  Write-Warning 'CutGeek is not installed, or was installed outside of Chocolatey. Nothing to do.'
}
else {
  Write-Warning "$($key.Count) matches found for CutGeek. Remove it manually rather than guessing:"
